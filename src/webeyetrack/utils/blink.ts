...
export function computeEAR(
    ...
    // Un-normalize ONLY the 6 points needed for this eye, converting to absolute pixels
    const pts = EYE_EAR_LANDMARKS.map(idx => [
        eyeLandmarks[idx].x * width,
        eyeLandmarks[idx].y * height
    ]);
